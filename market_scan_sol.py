#!/usr/bin/env python3
"""
Market scan + verdict for SOL perps on Hyperliquid. V2.
...
def load_klines_from_bybit(interval_minutes, limit=100):
    url = (
        f"https://api.bybit.com/v5/market/kline?category=linear&symbol=SOLUSDT"
...
def load_klines_from_okx(interval_minutes, limit=100):
    ...
    url = (
        f"https://www.okx.com/api/v5/market/candles?instId=SOL-USDT-SWAP"
...
def load_klines_from_hyperliquid(interval_minutes, limit=100):
    ...
    resp = http_json(
        ...
        body={
            "type": "candleSnapshot",
            "req": {"coin": "SOL", "interval": interval, "startTime": start_ms, "endTime": now_ms},
        },
    )
...
def maybe_send_alert(verdict_obj, mark):
    ...
    if v.get("verdict") == "GO":
        card = v.get("card", "?")
        params = v.get("params", {})
        alert_key = f"GO_{card}_{int(mark/100)}"  # cambia si SOL se mueve > $100
    ...
    if v.get("decision_zone") and v.get("preset_order"):
        ...
        message = (
            f"SOL ${mark:.0f} a {v.get('nearest_distance_pct')}% del trigger\n"
...
def select_verdict(mkt, regime, cards, long_sigs, short_sigs, ex_flags,
    ...
    if decision_zone and closest_card and cards[closest_card].get("params"):
        ...
        preset_order = {
            ...
            "note": "Orden pre-armada activada por decision_zone. Pegar en Valiant y dejar. Se ejecuta solo si SOL toca el trigger.",
        }
...
def main():
    ...
    ctxs = hl[1]
    idx = next(i for i, u in enumerate(universe) if u["name"] == "SOL")
    btc = ctxs[idx]
